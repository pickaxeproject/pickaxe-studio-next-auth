...
export const state = {
  async create(
    ...
    const actualValue = `value=${value};` + resParams.state;
    resParams.state = actualValue;
    const maxAge = options.cookies.state.options.maxAge ?? STATE_MAX_AGE
    cookies.push(await signCookie("state", actualValue, maxAge, options))
  },
  /**
  ...
  async use(
    ...
    const stateValue = value.value.split(";")[0].split("=")[1]

    checks.state = stateValue
  },
}
